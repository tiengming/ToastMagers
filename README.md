...
- **通知渠道服务关闭与锁定**：通过系统层级接口对目标应用特定 `NotificationChannel` 服务进行关停与锁定，防止应用运行时重建或重置渠道重要性。
- **通知拦截与统计分析机制**：采用无锁环形缓冲区在 Hook 路径异步收集拦截数据，提供多维度的应用拦截排行榜、日拦截趋势分析及日志/WebUI 可视化呈现。
...
- **WebUI 管理界面**：提供基于浏览器/内置 WebView 的可视化配置界面，覆盖应用清单浏览、规则开关、日志查看及拦截统计面板，替代直接编辑 `config.json` 的操作方式。**[待实现]**
- **系统权限状态同步**：规则开关状态与系统「设置 → 应用 → 通知」中的权限状态保持一致，避免系统 UI 显示与模块实际拦截行为不一致。**[待实现，详见下节]**
- **Systemless 部署与产物构建**：以 Magisk 模块形式挂载，支持自动化打包指令 `./gradlew zipModule` 生成发布包 `out/ToastMagers-release.zip`。
